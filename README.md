...
- **Two data sources on one radar**:
  - **Our network** (large dots) — propagation data aggregated from all Propagation Tracker users.
  - **PSK Reporter** (small dots) — worldwide reception reports from [pskreporter.info](https://pskreporter.info), queried directly by the client.
  - **Who heard me** (yellow stars) — stations that received *your* signal, via PSK Reporter callsign lookup.
- **Polar radar view** centered on your Maidenhead field, with In (red) / Out (blue) / Both (green) color coding, 15-minute and 3-hour trend layers.
- **PSK Reporter integration** (optional, toggle on/off):
  - Sequentially fetches Out → In → Who-heard-me, with automatic retry on rate limiting.
  - All bands cached; switching bands only filters the cache — no extra API requests.
  - Per-step status display: last data age, success/failure, countdown to next fetch.
- **Window transparency** toggle (0.5 / 1.0) so you can see through to other windows while monitoring.
...
- **雷达图上的两层数据**：大点 = 本网络汇聚数据；小点 = [PSK Reporter](https://pskreporter.info) 全球接收报告（客户端直连查询）；黄色五角星 =「谁听到我」。
- **极坐标雷达**：以你的 Maidenhead 场为中心，红/蓝/绿表示 In/Out/Both，支持 15 分钟与 3 小时趋势层。
- **PSK Reporter 集成**（可选勾选）：按序拉取 Out → In → 谁听到我，遇限流自动重试；全 band 缓存，切换 band 仅过滤不重请求；每步显示数据新鲜度、成功/失败、下次更新倒计时。
- **窗口透明度**：按钮在 0.5 与 1.0 之间切换，便于透过窗口看后面的内容。
...
- **レーダー上の二層データ**：大点＝本ネットワーク集約データ、小点＝[PSK Reporter](https://pskreporter.info) の全世界受信報告（クライアント直問合せ）、黄色星＝「誰が自分を聞いたか」。
- **極座標レーダー**：自分の Maidenhead フィールド中心、赤/青/緑で In/Out/Both、15分・3時間トレンド層対応。
- **PSK Reporter 連携**（オプション・チェックで有効）：Out → In → 誰が聞いたか を順に取得、レート制限時は自動リトライ。全バンドキャッシュ、バンド切替はフィルタのみ。各ステップのデータ鮮度・成否・次回取得までのカウントダウン表示。
- **ウィンドウ透過**：0.5 と 1.0 を切り替えるボタンで、背面のウィンドウを見ながら監視可能。
...
- **Zwei Datenquellen auf einem Radar**: Große Punkte = unser Netzwerk; kleine Punkte = [PSK Reporter](https://pskreporter.info) (weltweite Empfangsberichte, Client fragt direkt ab); gelbe Sterne = „Wer hat mich gehört“.
- **Polares Radar**: Zentriert auf Ihr Maidenhead-Feld, In/Out/Both in Rot/Blau/Grün, 15-Minuten- und 3-Stunden-Trend.
- **PSK-Reporter-Anbindung** (optional, per Checkbox): Sequenziell Out → In → Wer-hat-mich-gehört, bei Ratenbegrenzung automatischer Retry. Alle Bänder gecacht; Bandwechsel nur Filter, keine zusätzlichen API-Aufrufe. Pro Schritt: Datenalter, Erfolg/Misserfolg, Countdown bis zum nächsten Abruf.
- **Fenster-Transparenz**: Schalter zwischen 0,5 und 1,0, um dahinterliegende Fenster sichtbar zu lassen.
...
- **Dos fuentes de datos en un radar**: puntos grandes = nuestra red; puntos pequeños = [PSK Reporter](https://pskreporter.info) (informes mundiales, el cliente consulta directamente); estrellas amarillas = "quién me oyó".
- **Vista radar polar**: centrada en tu campo Maidenhead, In/Out/Both en rojo/azul/verde, capas de tendencia de 15 min y 3 h.
- **Integración PSK Reporter** (opcional, casilla): obtiene secuencialmente Out → In → quién me oyó; reintento automático si hay límite de tasa. Todas las bandas en caché; cambiar de banda solo filtra. Por paso: antigüedad de datos, éxito/fallo, cuenta atrás hasta la próxima actualización.
- **Transparencia de ventana**: botón para alternar entre 0,5 y 1,0 y ver ventanas detrás.
...
- **Два источника на одном радаре**: крупные точки = наша сеть; мелкие = [PSK Reporter](https://pskreporter.info) (мировые отчёты, запросы с клиента); жёлтые звёзды = «кто меня слышал».
- **Полярный радар**: центр — ваше поле Maidenhead, In/Out/Both красный/синий/зелёный, слои тренда 15 мин и 3 ч.
- **Интеграция PSK Reporter** (по желанию, галочка): последовательно Out → In → кто меня слышал; при лимите запросов — повтор. Все диапазоны в кэше; смена диапазона — только фильтр. По каждому шагу: свежесть данных, успех/ошибка, обратный отсчёт до следующего обновления.
- **Прозрачность окна**: кнопка переключения 0,5 / 1,0 — можно видеть окна сзади.
...
- **Deux sources sur un radar** : gros points = notre réseau ; petits points = [PSK Reporter](https://pskreporter.info) (rapports mondiaux, requêtes directes) ; étoiles jaunes = « qui m'a entendu ».
- **Radar polaire** : centré sur votre champ Maidenhead, In/Out/Both en rouge/bleu/vert, couches tendance 15 min et 3 h.
- **Intégration PSK Reporter** (optionnelle, case à cocher) : récupération séquentielle Out → In → qui m'a entendu ; nouvel essai automatique en cas de limite. Toutes les bandes en cache ; changement de bande = filtre uniquement. Par étape : fraîcheur des données, succès/échec, compte à rebours jusqu'à la prochaine mise à jour.
- **Transparence de la fenêtre** : bouton pour alterner 0,5 et 1,0 et voir les fenêtres derrière.
...
- **Duas fontes num radar**: pontos grandes = nossa rede; pontos pequenos = [PSK Reporter](https://pskreporter.info) (relatórios mundiais, consulta direta pelo cliente); estrelas amarelas = "quem me ouviu".
- **Radar polar**: centrado no seu campo Maidenhead, In/Out/Both em vermelho/azul/verde, camadas de tendência de 15 min e 3 h.
- **Integração PSK Reporter** (opcional, caixa de seleção): busca sequencial Out → In → quem me ouviu; nova tentativa automática em caso de limite de taxa. Todas as faixas em cache; troca de faixa só filtra. Por etapa: idade dos dados, sucesso/falha, contagem regressiva até a próxima atualização.
- **Transparência da janela**: botão para alternar entre 0,5 e 1,0 e ver janelas atrás.
