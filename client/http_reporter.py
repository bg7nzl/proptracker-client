...
from .auth import make_auth_token
...
class HttpReporter:
    ...
    def _check_health(self) -> bool:
        try:
            req = Request(_cfg.HEALTH_ENDPOINT, method="GET")
    ...
    def _worker(self) -> None:
        ...
        backoff = 1.0
        while not self._stop.wait(timeout=min(1.0, _cfg.REPORT_INTERVAL_SEC)):
            now = time.monotonic()
            if now - self._last_health_check >= HEALTH_CHECK_INTERVAL_SEC:
                ...
                if self._check_health():
                    ...
                    self._is_connected = False
            while len(batch) < _cfg.REPORT_BATCH_SIZE:
                try:
                    ...
                    break
            if batch and (
                len(batch) >= _cfg.REPORT_BATCH_SIZE or (now - last_send) >= _cfg.REPORT_INTERVAL_SEC
            ):
    ...
    def _send_batch(self, reports: list[PropagationReport]) -> bool:
        body = json.dumps(
            {"client_version": _cfg.CLIENT_VERSION, "reports": [r.to_dict() for r in reports]}
        ).encode("utf-8")
        headers = {"Content-Type": "application/json"}
        if self._api_key:
            token = make_auth_token(self._api_key, body)
            headers["Authorization"] = f"FT8Auth {token}"
        req = Request(
            _cfg.SERVER_ENDPOINT,
            data=body,
            headers=headers,
            method="POST",
        )
